...
//
//You should calculate dp[x] in the increasing order of x. The value of dp[x] is (maximal value of d[i] where i is a multiple of x) + 1. After you calculate dp[x], for each divisor i of x, you should update d[i] too.
//
//This algorithm works in O(nlogn) because the sum of the number of the diviser from 1 to n is O(nlogn).
//
...
#define lim 100010
int n;
int arr[lim],dp[lim],d[lim];
VI divi[lim];

int makedp(int x)
{
    int i;
    for(i=0;i<SZ(divi[x]);i++)
        dp[x]=max(dp[x],d[divi[x][i]]+1);
    for(i=0;i<SZ(divi[x]);i++)
        d[divi[x][i]]=dp[x]; //automatically max
    //print2(x,dp[x]);
    return dp[x];
}
