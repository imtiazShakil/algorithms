...
///finds the maximum value in an array within a given range
...
{
    if(i>rght || j<lft) return -1; /// lft & rght are outside the range< i , j>

    if( lft>=i && rght<=j ) return s_tree[node]; /// lft & rght is inside <i ,j>

    int mx=query(node*2 , lft, (lft+rght)/2 , i,j );
    mx=max(mx,query(node*2 + 1, (lft+rght)/2 + 1,rght, i, j) );
    return mx;
}
